...
options["camerlength"] = 120
options["exptime"] = 0.5

options["wavelength"] = 0.04176
options["rotationaxis"] =  -143
options["strechamp"] = 0
options["strechazimuth"] = 0
options["direction"] = 0
...
def run(options, path, gear, CRS, cameraLenght, MRC, fMRC, IMG, smooth, fixDist,
        quiet, startangle, direction, expttime, factor, tiff, n_add_tiff, interpolate):
    ...
    options["inter_cross"] = interpolate.get()
...
def main(options):
    ...
    ttk.OptionMenu(mainframe, cameraLenght, options["camerlength"], 30, 40, 50, 60, 80,
                   100, 120, 150, 200).grid(column=2, row=3, sticky=W)
    ...
    interpolate = tk.BooleanVar()
    interpolate.set(options["inter_cross"])

    ttk.Checkbutton(mainframe, text="interpolate", variable=interpolate).grid(
